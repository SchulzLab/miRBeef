...
def main(args):
    ...
    objective_log_dir = os.path.join(log_root, "objective")
    ...
    process = psutil.Process()  # Initialize process monitoring here for the entire script
    ...
    for fold, (train_val_indices, test_indices) in enumerate(skf.split(dataset, labels)):
        ...
        def objective(trial):
            ...
            # Sample optimizer and its parameters
            ...
            trial_batch_size = trial.suggest_categorical('batch_size', [50, 100, 200])
            ...
            print(f"Fold {fold}, Trial {trial.number} sampled: optimizer={opt_name}, dropout_rate={dropout_rate}, batch_size={trial_batch_size}, opt_config={opt_config}")

            # Reset memory tracking before training
            if torch.cuda.is_available():
                torch.cuda.reset_max_memory_allocated()

            train_loader = dataloader(train_subset_trial, batch_size=trial_batch_size, shuffle=True,
                                      num_workers=args.num_workers, pin_memory=True)
            val_loader = dataloader(val_subset_trial, batch_size=trial_batch_size, shuffle=False,
                                    num_workers=args.num_workers, pin_memory=True)
            ...
            simple_logger = CSVLogger(objective_log_dir, name=f"fold{fold}_trial{trial.number}")
            trainer = L.Trainer(
            ...
            mem_before = process.memory_info().rss / (1024 ** 2)  # Memory in MB before training
            ...
            mem_after = process.memory_info().rss / (1024 ** 2)  # Memory in MB after training
            cpu_percent = psutil.cpu_percent(interval=None)  # Peak CPU usage during the last operation
        ...
        storage_url = f"sqlite:///mitar_study_fold_{fold}.db"
        ...
        # Final Model Training and Testing (getting prepared)
        ...
        train_loader = dataloader(train_subset, batch_size=final_batch_size, shuffle=True,
                                  num_workers=args.num_workers, pin_memory=True)
        val_loader = dataloader(val_subset, batch_size=final_batch_size, shuffle=False,
                                num_workers=args.num_workers, pin_memory=True)
        ...
        test_loader = dataloader(test_subset, batch_size=test_batch_size, shuffle=False,
                                 num_workers=args.num_workers, pin_memory=False)  # pin_memory=False for CPU
        ...
        final_logger = (CometLogger(
            api_key="-",
            project_name="MiTar_on_CLIP_dataset",
            workspace="-"
        ) if args.comet_logging else CSVLogger(log_root, name=save_name))
        
        # creating the checkpoint skeleton
        ...
        # using the skeleton # Final Model Training and fitting 
        final_trainer = L.Trainer(
            logger=final_logger,
            default_root_dir=os.path.join(CHECKPOINT_PATH, save_name),
            accelerator="auto",  # Use GPU if available for training
            devices="auto",
            inference_mode=False,
            max_epochs=args.epochs,
            callbacks=[
        ...
        if args.prediction_data_path is not None:
            log.info(f"Predicting for fold {fold} on CPU...")
            ...
            prediction_loader = DataLoader(prediction_dataset, batch_size=test_batch_size, shuffle=False,
                                           num_workers=args.num_workers, pin_memory=False)  # pin_memory=False for CPU
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num-trials', type=int, default=100, help='Number of Optuna trials per fold')
    parser.add_argument('--val-ratio', type=float, default=0.1, help='Validation split ratio from the train_val set')
    parser.add_argument('--model', type=str, default='MiTar',
                        choices=['CNNSequenceModel', 'DMISO', 'MiTar', 'TransPHLA', 'TEC-miTarget', 'GraphTar'],
                        help='Model name')
    parser.add_argument('--comet-logging', action='store_true', help='Log to Comet.ml')
    parser.add_argument('--dataset', type=str, default='MiTar', help='Dataset to use',
    ...
    parser.add_argument('--canonical-only', action='store_true', help='Use only canonical miRNA-target pairs')
    ...
    parser.add_argument('--word2vec-model-dir', type=str, default='data/mitar/word2vec_models', help='Word2Vec directory')
    parser.add_argument('--num-workers', type=int, default=16, help='Number of DataLoader workers')
    parser.add_argument('--log-dir', type=str, default='mitar_log_CLIP', help='Directory for log files and checkpoints')
